...
interface BundleFormRow {
  ...
  bundleNo: string;
  bundleQty: string;
  size: string;         // free text now
  numberRange: string;
}

// A saved cut in the cut summary table
interface SavedCut {
  tempId: string;
  cutNo: string;        // e.g. "Front Cut 1"
  component: string;    // e.g. "Front"
  cutQty: number;
...
interface StagedEntry {
  ...
  inQty: number;
  cuts: { cutNo: string; cutQty: number; bundles: { bundleNo: string; bundleQty: number; size: string; numberRange: string }[] }[];
}
...
export default function StoreInPage() {
  ...
  // ── Cut builder (active cut being filled) ────────────────────────────────
  ...
  const [editingCutTempId, setEditingCutTempId] = useState<string | null>(null); // null = new cut

  // ── Cut summary table (cuts saved within this entry, not yet in DB) ───────
  const [savedCuts, setSavedCuts] = useState<SavedCut[]>([]);

  // ── Staging table (multiple schedule entries) ─────────────────────────────
  ...
  // ── Derived: unique StyleNos ──────────────────────────────────────────────
  const uniqueStyleNos = useMemo(() =>
    Array.from(new Set(eligibleStoreInItems.map(i => i.styleNo))).sort(),
    [eligibleStoreInItems]);

  // ── Derived: colour variants for selected style ───────────────────────────
  const colourVariants = useMemo(() =>
    !selectedStyleNo ? [] : eligibleStoreInItems.filter(i => i.styleNo === selectedStyleNo),
    [eligibleStoreInItems, selectedStyleNo]);

  // ── Derived: selected eligible item ──────────────────────────────────────
  const selectedItem = useMemo(() =>
    eligibleStoreInItems.find(i => i.submissionId === submissionId) || null,
    [eligibleStoreInItems, submissionId]);

  // ── Derived: ordered list of components for this style ───────────────────
  ...
  // ── Derived: next component for the active cut ───────────────────────────
  // If editing an existing cut, use its component. Otherwise assign next in order.
  const nextComponent = useMemo(() => {
    if (editingCutTempId) {
      return savedCuts.find(c => c.tempId === editingCutTempId)?.component ?? '';
    }
    // How many cuts have been saved so far → pick next component cyclically
    const idx = savedCuts.length % Math.max(componentList.length, 1);
    return componentList[idx] ?? `Cut ${savedCuts.length + 1}`;
  }, [editingCutTempId, savedCuts, componentList]);

  // ── Derived: cut title ────────────────────────────────────────────────────
  const activeCutTitle = useMemo(() => {
    const globalCutNum = savedCuts.length + 1;
    if (editingCutTempId) {
      const existing = savedCuts.find(c => c.tempId === editingCutTempId);
      return existing ? existing.cutNo : `${nextComponent} Cut`;
    }
    return nextComponent ? `${nextComponent} Cut ${globalCutNum}` : `Cut ${globalCutNum}`;
  }, [editingCutTempId, savedCuts, nextComponent]);
  ...
  const existingScheduleOptions = useMemo(() => {
    ...
    const inSystem = systemStyleSchedules
      .filter(r => r.styleNo === selectedItem.styleNo && r.customerName === selectedItem.customerName && !!r.scheduleNo)
      .map(r => r.scheduleNo.trim());
    const inRecords = storeInRecords
      .filter(r => r.submissionId === submissionId && !!r.scheduleNo)
      .map(r => r.scheduleNo.trim());
  ...
  const draftState = useMemo(() => ({
    selectedStyleNo, selectedBodyColour, submissionId,
    scheduleNo, cutInDate, inQty, savedCuts, stagedEntries,
  }), [selectedStyleNo, selectedBodyColour, submissionId, scheduleNo, cutInDate, inQty, savedCuts, stagedEntries]);
  ...
  const handleStyleNoChange = (styleNo: string) => {
    ...
    setScheduleNo('');
    setSavedCuts([]);
    setErrors(p => ({ ...p, styleNo: '', bodyColour: '', submissionId: '' }));
    setPageError('');
  };
  ...
  const handleBodyColourChange = (bodyColour: string) => {
    ...
    setSubmissionId(match?.submissionId ?? '');
    setSavedCuts([]);
    setErrors(p => ({ ...p, bodyColour: '', submissionId: '' }));
    setPageError('');
  };
  ...
  const removeBundle = (tempId: string) =>
    setActiveCutBundles(prev => prev.filter(b => b.tempId !== tempId));

  const updateBundle = (tempId: string, field: keyof BundleFormRow, value: string) =>
    setActiveCutBundles(prev => prev.map(b => b.tempId === tempId ? { ...b, [field]: value } : b));
  ...
  const validateActiveCut = (): boolean => {
    ...
    if (cutQtyNum > 0 && totalBundleQty !== cutQtyNum)
      errs.bundles = totalBundleQty > cutQtyNum
        ? `Bundle total (${totalBundleQty}) exceeds cut qty (${cutQtyNum})`
        : `Bundle total (${totalBundleQty}) must equal cut qty (${cutQtyNum}) — unbundled: ${cutQtyNum - totalBundleQty}`;

    const bundleNos = activeCutBundles.map(b => b.bundleNo.trim().toLowerCase());
    if (bundleNos.length !== new Set(bundleNos).size)
      errs.bundles = 'Duplicate bundle numbers found';
  ...
  // ── Save active cut to summary table ─────────────────────────────────────
  const handleSaveCut = () => {
  ...
  const validateForm = (): boolean => {
    const errs: Record<string, string> = {};
    if (!selectedStyleNo)    errs.styleNo     = 'Please select a style number';
    if (!selectedBodyColour) errs.bodyColour  = 'Please select a body colour';
    if (!submissionId)       errs.submissionId = 'No approved submission found';
    if (!scheduleNo.trim())  errs.scheduleNo  = 'Schedule No is required';
    if (!cutInDate)          errs.cutInDate   = 'Cut In Date is required';
    if (inQtyNum <= 0)       errs.inQty       = 'IN Qty must be greater than 0';
    else if (inQtyNum > remainingBulk && !editingRecordId)
      errs.inQty = `Exceeds remaining bulk balance (${remainingBulk})`;
    if (savedCuts.length === 0) errs.cuts = 'Add at least one cut before saving';
    if (totalCutQty > inQtyNum && inQtyNum > 0)
      errs.cuts = `Total cut qty (${totalCutQty}) exceeds IN qty (${inQtyNum})`;
    setErrors(errs);
    return Object.keys(errs).length === 0;
  };
  ...
  const handleSaveToTable = (e: React.FormEvent) => {
    ...
    setSuccessMsg('Added to staging table. Add more schedules or save all to database.');
    setTimeout(() => setSuccessMsg(''), 4000);

    // Keep style + colour, reset per-entry fields
    setScheduleNo('');
    setCutInDate('');
    setInQty('');
    setSavedCuts([]);
  ...
  const handleDirectSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateStoreInRecord(editingRecordId!, {
        ...
        cutInDate,
        inQty: inQtyNum,
        cuts: savedCuts.map(c => ({ cutNo: c.cutNo, cutQty: c.cutQty, bundles: c.bundles })),
      });
      resetForm();
      await Promise.all([fetchRecords(), fetchEligibleStoreInItems(), fetchBulkBalances()]);
  ...
  const handleEdit = (record: StoreInRecord) => {
    ...
    setSelectedBodyColour(record.bodyColour);
  ...
  const scheduleDatalistId = `sched-${submissionId || 'none'}`;
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mx-auto max-w-6xl space-y-8 pb-12">
      ...
      <div className="flex items-center space-x-3 border-b border-slate-200 pb-4">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">Receive goods against approved styles. Each cut is named after its component (Front, Back…).</p>
        </div>
      </div>
        ...
        <form onSubmit={editingRecordId ? handleDirectSubmit : handleSaveToTable} className="space-y-6">
          ...
          <div className="rounded-lg border border-blue-200 bg-blue-50/60 p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="space-y-1">
                <label className="block text-xs font-medium text-slate-600">Body Colour <span className="text-red-500">*</span></label>
                <select value={selectedBodyColour} onChange={e => handleBodyColourChange(e.target.value)} disabled={!!editingRecordId || !selectedStyleNo}
                  className={`w-full rounded-lg border px-3 py-2 text-sm outline-none bg-white ${errors.bodyColour ? 'border-red-400 bg-red-50' : 'border-slate-300 focus:ring-2 focus:ring-blue-500'} ${(!selectedStyleNo || editingRecordId) ? 'cursor-not-allowed bg-slate-100 text-slate-400' : ''}`}>
                  <option value="">{!selectedStyleNo ? 'Select style first...' : 'Select body colour...'}</option>
                  {colourVariants.map(item => (
                    <option key={item.submissionId} value={item.bodyColour}>
                      {item.bodyColour} — Remaining: {item.remainingBulkQty}
                    </option>
                  ))}
                </select>
              ...
              <div className="space-y-1">
                ...
                <input type="text" list={scheduleDatalistId} value={scheduleNo}
                  ...
                  disabled={!selectedItem} placeholder={selectedItem ? 'Type or pick…' : 'Select colour first…'} autoComplete="off"
                  className={`w-full rounded-lg border bg-white px-3 py-2 text-sm outline-none ${errors.scheduleNo ? 'border-red-400 bg-red-50' : 'border-slate-300 focus:ring-2 focus:ring-blue-500'} ${!selectedItem ? 'cursor-not-allowed bg-slate-100 text-slate-400' : ''}`}
                />
            ...
            {selectedItem && (
              <div className="grid grid-cols-2 gap-3 md:grid-cols-4 mt-2">
                ...
                <ReadOnlyField label="Approved Bulk Qty" value={selectedItem.approvedBulkQty.toString()} highlight />
                {selectedItem.components && (
                  <div className="col-span-2 md:col-span-4 space-y-0.5">
                    <label className="block text-[10px] font-medium uppercase tracking-wide text-slate-400">Components</label>
                    <div className="flex flex-wrap gap-1.5">
                      {componentList.map(comp => (
                        <span key={comp} className="inline-block rounded-full border border-indigo-200 bg-indigo-50 px-2.5 py-0.5 text-xs font-medium text-indigo-700">{comp}</span>
                      ))}
                    </div>
                  </div>
                )}
          ...
          <div className="rounded-lg border border-orange-200 bg-orange-50/60 p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              <div className="space-y-1">
                ...
                <input type="number" value={inQty}
                  onChange={e => { setInQty(e.target.value); if (errors.inQty) setErrors(p => ({ ...p, inQty: '' })); }}
              ...
              <div className="space-y-1">
                ...
                <div className={`rounded-lg border px-3 py-2 text-sm font-black ${remainingBulk > 0 ? 'border-blue-200 bg-blue-50 text-blue-700' : 'border-emerald-200 bg-emerald-50 text-emerald-700'}`}>
                  {selectedItem ? remainingBulk : '-'}
                </div>
              </div>
              <div className="space-y-1">
                <label className="block text-xs font-medium text-slate-500">Uncut Balance</label>
                <div className={`rounded-lg border px-3 py-2 text-sm font-bold ${uncutBalance > 0 ? 'border-amber-200 bg-amber-50 text-amber-700' : 'border-slate-200 bg-white text-slate-500'}`}>
          ...
          <div className="space-y-4">
            ...
            {savedCuts.length > 0 && (
              <div className="rounded-lg border border-slate-200 overflow-hidden">
                <div className="bg-slate-50 px-4 py-2 border-b border-slate-200 flex items-center justify-between">
                  ...
                  <span className="text-xs text-slate-500">Total cut qty: <span className="font-bold text-slate-800">{totalCutQty}</span></span>
                </div>
                          ...
                          {editingCutTempId === cut.tempId ? (
                            ...
                            <>
                              <button type="button" onClick={() => handleEditSavedCut(cut)}
                                className="rounded p-1.5 text-slate-400 hover:bg-blue-50 hover:text-blue-600 transition-colors" title="Edit cut">
                                <Pencil className="h-3.5 w-3.5" />
                              </button>
                              <button type="button" onClick={() => handleRemoveSavedCut(cut.tempId)}
                                className="rounded p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-500 transition-colors" title="Remove cut">
                        ...
                        <table className="w-full text-xs">
                          <thead>
                            <tr className="text-slate-400">
                              <th className="py-0.5 text-left font-medium w-24">Bundle</th>
                              <th className="py-0.5 text-left font-medium w-16">Qty</th>
                              <th className="py-0.5 text-left font-medium w-16">Size</th>
                              <th className="py-0.5 text-left font-medium">Range</th>
                            </tr>
                          </thead>
                          <tbody>
                            {cut.bundles.map((b, i) => (
            ...
            <AnimatePresence mode="wait">
              <motion.div key={editingCutTempId ?? 'new'}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <h5 className={`text-sm font-bold ${editingCutTempId ? 'text-amber-800' : 'text-indigo-800'}`}>
                      {editingCutTempId ? `Editing: ${savedCuts.find(c => c.tempId === editingCutTempId)?.cutNo}` : activeCutTitle}
                    </h5>
                    {!editingCutTempId && nextComponent && (
                      <span className="text-[10px] font-medium text-indigo-500 bg-indigo-100 px-2 py-0.5 rounded-full">
                        Component: {nextComponent}
                      </span>
                    )}
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2">
                      <label className="text-xs font-medium text-slate-600">Cut Qty <span className="text-red-500">*</span></label>
                ...
                {cutErrors.cutQty && <p className="text-[11px] text-red-600"><AlertCircle className="mr-1 inline h-3 w-3" />{cutErrors.cutQty}</p>}
                  ...
                  {activeCutBundles.map((bundle, bi) => (
                    <div key={bundle.tempId} className="grid grid-cols-12 gap-2 items-start">
                      ...
                      <div className="col-span-2">
                        <input type="number" value={bundle.bundleQty}
                          onChange={e => updateBundle(bundle.tempId, 'bundleQty', e.target.value)}
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-slate-200">
                  ...
                  {!editingCutTempId && savedCuts.length > 0 && (
                    <span className="text-xs text-slate-400 ml-2">
                      Next: <span className="font-semibold text-slate-600">
                        {componentList[(savedCuts.length) % Math.max(componentList.length, 1)] || `Cut ${savedCuts.length + 2}`}
                      </span> cut
                    </span>
                  )}
                </div>
            ...
            {stagedEntries.map(entry => {
              ...
              return (
                <div key={entry.tempId}>
                  <div className="flex items-center gap-4 px-6 py-4 hover:bg-amber-50 cursor-pointer transition-colors"
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-indigo-600 font-medium">· {entry.bodyColour}</span>
                      </div>
                      <p className="text-xs text-slate-500 mt-0.5">Sch: {entry.scheduleNo} | Date: {entry.cutInDate}</p>
                    </div>
                    ...
                    <button onClick={e => { e.stopPropagation(); removeStagedEntry(entry.tempId); }}
                      className="rounded p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-500 transition-colors shrink-0">
                        ...
                        {entry.cuts.map(cut => (
                          <div key={cut.cutNo} className="mb-3 last:mb-0">
                            ...
                            <div className="ml-6 border-l-2 border-slate-200 pl-4">
                              <table className="w-full text-xs">
                                <thead><tr className="text-slate-400"><th className="py-1 text-left font-medium">Bundle</th><th className="py-1 text-left font-medium">Qty</th><th className="py-1 text-left font-medium">Size</th><th className="py-1 text-left font-medium">Range</th></tr></thead>
        ...
        <div className="border-b border-slate-200 bg-slate-50 px-6 py-4 space-y-3">
          ...
          <div className="relative max-w-sm">
            ...
            <input type="text" value={searchText} onChange={e => { setSearchText(e.target.value); setCurrentPage(1); }}
              placeholder="Search by style, customer, schedule, colour..."
                ...
                <div className="flex flex-wrap items-end gap-3 pt-2">
                  <div className="space-y-1">
                    ...
                    <input type="date" value={filterDateFrom} onChange={e => { setFilterDateFrom(e.target.value); setCurrentPage(1); }}
                      className="rounded-lg border border-slate-200 px-3 py-1.5 text-sm outline-none focus:ring-1 focus:ring-blue-400" />
                  </div>
                  <div className="space-y-1">
                  ...
                  <button type="button" onClick={() => { setFilterDateFrom(''); setFilterDateTo(''); setSearchText(''); setCurrentPage(1); }}
                    className="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50">Clear All</button>
                  ...
                  <div className="flex items-center gap-4 px-6 py-4 hover:bg-slate-50/50 cursor-pointer transition-colors"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">
                        {record.bodyColour} · Sch: {record.scheduleNo} | Date: {record.cutInDate} | {record.season}
                      </p>
                    </div>
                    <div className="text-right space-y-0.5 shrink-0">
                      ...
                      {record.uncutBalance > 0 && (
                        <span className="rounded-full bg-amber-100 border border-amber-300 px-2 py-0.5 text-[10px] font-bold text-amber-700">
                          Uncut: {record.uncutBalance}
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-1 shrink-0" onClick={e => e.stopPropagation()}>
                      {locks[record.id]?.isLocked ? (
                        <div className="flex items-center gap-1" title="Locked — downstream data exists">
                          <Lock className="h-4 w-4 text-slate-300" /><span className="text-[10px] text-slate-400">Locked</span>
                        </div>
                      ) : (
                        <>
                          <button onClick={() => handleEdit(record)} className="rounded p-1.5 text-slate-400 hover:bg-blue-50 hover:text-blue-600 transition-colors"><Edit2 className="h-4 w-4" /></button>
                          <button onClick={() => handleDelete(record.id)} className="rounded p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-500 transition-colors"><Trash2 className="h-4 w-4" /></button>
                        </>
                      )}
                    </div>
                  </div>
...
function ReadOnlyField({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="space-y-0.5">
      ...
      <p className={`text-sm font-medium ${highlight ? 'text-blue-700 font-bold' : 'text-slate-700'}`}>{value || '-'}</p>
    </div>
  );
}
